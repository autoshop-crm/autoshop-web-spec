...
# 5.5 Unified поиск: склад + внешняя база

Для сценария единого списка теперь добавлен новый endpoint:
- `GET /api/parts/unified/search`

## Назначение
Этот endpoint нужен, когда frontend хочет:
- искать локальные складские позиции и внешние аналоги одним запросом;
- показывать единый список результатов;
- сразу понимать, есть ли у внешней найденной детали локальная складская позиция;
- не реализовывать маппинг `external -> local warehouse part` на frontend вручную.

## Доступ
Это `GET` под `/api/parts/**`, значит доступно ролям:
- `ADMIN`
- `MANAGER`
- `RECEPTIONIST`
- `MECHANIC`
...
- `limit` — optional, относится к внешнему поиску
- `offset` — optional, относится к внешнему поиску

## Что делает backend
Backend делает 2 поиска внутри одного вызова:
1. ищет локальные запчасти на складе через текущий `PartService.search(...)`;
2. ищет внешние аналоги через `GET /api/parts/external/search` внутренней сервисной логикой.

После этого backend объединяет результаты в единый список и для каждого внешнего результата пытается найти локальную складскую позицию по нормализованному `articleNumber`.

То есть теперь на backend есть готовый серверный контракт:
- внешний item;
- признак, найден ли точный local match;
- сама локальная складская позиция, если она существует.
...
### `sourceType = LOCAL`
Это локальная складская позиция из Core.
- основное содержимое находится в `localPart`;
- `matchedLocalPart` совпадает с ней же.

### `sourceType = EXTERNAL`
Это внешняя найденная деталь из UMAPI.
- основное содержимое находится в `externalPart`;
- если backend нашёл точную локальную позицию по артикулу, она лежит в `matchedLocalPart`;
- `exactLocalMatch = true` означает, что внешний item уже имеет соответствие на локальном складе;
- `availableLocally = true` означает, что локально эта позиция не просто существует, но и доступна по остатку.

## Что это решает
Раньше в backend не было готового endpoint:
- “найти аналог и сразу привязать к локальному складу”

Теперь такой контракт есть.
Frontend может:
- рендерить один объединённый список;
- показывать бейджи `На складе`, `Есть локальный аналог`, `Только внешний`;
- не делать отдельный дополнительный запрос для сопоставления внешнего результата со складом.

## Рекомендуемая frontend-логика

### Единый экран поиска запчастей
1. пользователь вводит `articleNumber` и при желании `brand`;
2. frontend вызывает `GET /api/parts/unified/search`;
3. получает единый список `items`;
4. отображает:
   - локальные позиции;
   - внешние позиции;
   - признак наличия локального совпадения;
   - признак доступности на складе.

### Рекомендуемое отображение
- `LOCAL` item — показывать как складскую карточку;
- `EXTERNAL` item с `matchedLocalPart != null` — показывать как внешний аналог с привязкой к складу;
- `EXTERNAL` item без `matchedLocalPart` — показывать как только внешний результат.

### Полезные UI-бейджи
- `LOCAL`
- `EXTERNAL`
- `Есть на складе`
- `Есть локальное совпадение`
- `Только внешний`

## Ограничения текущей реализации
- сопоставление внешней детали с локальным складом сейчас идёт по нормализованному `articleNumber`;
- это не “умный” аналоговый подбор по брендам, кроссам и substitute relations;
- то есть это именно **server-side exact article mapping**, а не глубокий аналоговый matching engine.

Но для frontend это уже закрывает ключевую задачу:
- получить общий список и понять, что из внешнего уже есть локально.
